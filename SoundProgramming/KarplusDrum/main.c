...
//  KarplusDrum
//
//  Created by Kazuya Shida on 2017/11/20.
...
int main(int argc, const char * argv[])
{
    ...
    for (int i = wavt_len + 1; i < n_data; i++) {
        buffer[i] = ((double)rand()/(double)RAND_MAX > b ? 0.5 : -0.5) * (buffer[i - wavt_len] + buffer[i - wavt_len - 1]);
        bufhead.data[i] = (short) buffer[i];
    }
